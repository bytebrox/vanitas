...
/**
 * Sui difficulty readout — hex alphabet (16)
 */
...
interface SuiDifficultyDisplayProps {
  prefix: string;
  suffix: string;
...
export function SuiDifficultyDisplay({
  prefix,
  suffix,
  patterns,
  currentRate,
}: SuiDifficultyDisplayProps) {
  const estimatedRate = useMemo(() => {
    if (currentRate > 0) return currentRate;
    const cores = typeof navigator !== 'undefined' ? navigator.hardwareConcurrency || 4 : 4;
    const workers = Math.max(1, cores - 1);
    // Ed25519 + blake2b is typically fast in JS
    return workers * 8000;
  }, [currentRate]);
  ...
  const difficulty = useMemo(
    () => combineOrDifficulty(targets.map((p) => estimateSuiDifficulty(p.prefix, p.suffix))),
    [targets]
  );

  const difficultyLabel = useMemo(() => formatSuiDifficulty(difficulty), [difficulty]);
  const timeEstimate = useMemo(
    () => estimateSuiTime(difficulty, estimatedRate),

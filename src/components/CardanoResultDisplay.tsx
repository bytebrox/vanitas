...
export function CardanoResultDisplay({ result, onReset, onContinueSearch }: Props) {
  const t = useTranslations('common');
  const [showPrivateKey, setShowPrivateKey] = useState(false);
  const [copiedField, setCopiedField] = useState<string | null>(null);
  const shortId = result.address.slice(5, 13);
  ...
  const downloadTxt = () => {
    const content = buildVanityExportTxt(t, {
      title: `${t('exportTxtTitle')} — CARDANO`,
    ...
    a.href = url;
    a.download = `cardano-${shortId}.txt`;
  ...
  const downloadJson = () => {
    const payload = {
      ...
      importHint: 'Ed25519 payment key (hex). Enterprise address has no stake credential.',
    };
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `cardano-${shortId}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  return (
    <div className="space-y-10">
      ...
      <PostFindPlaybook chain="cardano" mode={'wallet'} address={result.address} />
      <ImportGuide chain="cardano" mode={'wallet'} />

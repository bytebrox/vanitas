/**
 * Validation for Aptos hex vanity patterns
 */

import type { ValidationResult } from '@/types';

export function isValidAptosHex(str: string): boolean {
  return /^[0-9a-fA-F]*$/.test(str);
}

export function validateAptosPrefix(prefix: string): ValidationResult {
...
export function validateAptosSuffix(suffix: string): ValidationResult {
 ...
 */
export function estimateAptosDifficulty(prefix: string, suffix: string): number {
  const total = prefix.length + suffix.length;
  if (total === 0) return 1;
  return Math.pow(16, total);
}

export function formatAptosDifficulty(difficulty: number): string {
...
export function estimateAptosTime(difficulty: number, ratePerSecond: number): string {
...
export function normalizeAptosPattern(value: string): string {

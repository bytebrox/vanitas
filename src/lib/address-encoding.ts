/**
 * Shared address encodings for Bitcoin, Tron, Cardano & XRP (browser + workers)
 */
...
export function xrpBase58Encode(bytes: Uint8Array): string {
  ...
  // Leading zero bytes encode as the first alphabet character ('r')
...
/**
 * XRPL classic account address (`r…`) from a 33-byte compressed secp256k1
 * public key (or 33-byte Ed25519 key with 0xED prefix).
 * Account ID = RIPEMD160(SHA256(pubkey)); type prefix 0x00.
 */

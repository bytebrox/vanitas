/**
 * Sui vanity Web Worker — Ed25519 + blake2b(0x00 ‖ pubkey, dkLen: 32)
 */
...
/** Sui Ed25519 address: blake2b(0x00 ‖ pubkey, { dkLen: 32 }) */
function suiAddressFromPubkey(pub: Uint8Array): string {
  const preimage = new Uint8Array(33);
  preimage[0] = 0x00;
  preimage.set(pub, 1);
  return '0x' + etc.bytesToHex(blake2b(preimage, { dkLen: 32 }));
}
  ...
  switch (type) {
    case 'start':
      if (config && id !== undefined) {
        workerId = id;
        try {
          await generateSuiVanity(config);
        } catch (err) {
          self.postMessage({
            type: 'error',
            workerId,
